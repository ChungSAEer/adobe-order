...
/**
 * Ensure Sheet2 headers exist
 */
async function ensureSheet2Headers(sheets) {
  const sheetId = process.env.GOOGLE_SHEET_ID;
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: sheetId,
    range: 'Sheet2!A1:D1',
...
async function getAllLinkRows(sheets) {
  ...
  return rows.slice(1).map((row, i) => ({
    ...
    rowIndex: i + 2, // 1-indexed, +2 because slice(1) and header
  }));
}
...
export async function createLink(uid, duration) {
  const sheets = getSheetsClient();
  await ensureSheet2Headers(sheets);
  const sheetId = process.env.GOOGLE_SHEET_ID;
  const now = new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' });
...
export async function deleteLink(uid) {
  ...
  // Get sheet internal ID for Sheet2
  const meta = await sheets.spreadsheets.get({ spreadsheetId: sheetId });
  const sheet2 = meta.data.sheets.find((s) => s.properties.title === 'Sheet2');
  if (!sheet2) throw new Error('Sheet2 not found');
